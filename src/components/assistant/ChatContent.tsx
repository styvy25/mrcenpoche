      ...
      {messages.length === 0 ? <div className="flex h-full items-center justify-center bg-zinc-50 rounded">
        ...
        duration: 0.6
          ...
          repeat: Infinity
            ...
            <motion.div className="grid grid-cols-1 gap-2 text-sm" initial="hidden" animate="visible" variants={{
          visible: {
            transition: {
              staggerChildren: 0.1
            }
              ...
              {["Explique-moi les principaux objectifs du MRC", "Quelles sont les dernières actualités concernant le MRC ?", "Comment puis-je m'impliquer davantage dans le mouvement ?", "Trouve-moi des vidéos sur Maurice Kamto"].map((suggestion, index) => <motion.button key={index} className="p-2 rounded-md bg-gray-700/50 hover:bg-gray-700 text-left transition flex items-center group" variants={{
            hidden: {
              opacity: 0,
              y: 5
            },
          ...
          {messages.map((message, index) => <React.Fragment key={index}>
              <FuturisticMessage message={message} index={index} />
              
              {shouldShowAdAfterMessage(index) && <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} transition={{
          ...
          {youtubeResults && youtubeResults.length > 0 && <YouTubeResults results={youtubeResults} isLoading={isSearchingYouTube} onSelect={onVideoSelect} downloadLinks={downloadLinks} />}
          
          {downloadLinks && <div className="p-4 rounded-lg bg-gray-800/50 border border-gray-700 mt-4">
              ...
              <div className="flex flex-wrap gap-2">
                {downloadLinks.downloadServices && downloadLinks.downloadServices.map((service: any, idx: number) => <Button key={idx} variant="outline" size="sm" className="flex items-center bg-gray-800/80 hover:bg-gray-700" onClick={() => window.open(service.url, '_blank')}>
                    <FileDown className="h-4 w-4 mr-1" /> {service.name}
                  </Button>)}
                {downloadLinks.watchUrl && <Button variant="outline" size="sm" className="flex items-center bg-gray-800/80 hover:bg-gray-700" onClick={() => window.open(downloadLinks.watchUrl, '_blank')}>
                    Voir sur YouTube
                  </Button>}
              </div>
            </div>}
          ...
          {messages.length >= 2 && <motion.div initial={{
        opacity: 0
      }} animate={{
        opacity: 1
      }} transition={{
