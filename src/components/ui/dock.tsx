...
export function Dock({
  ...
  return <DockContext.Provider value={{
    ...
    setActiveIndex
      ...
      <div className={cn("flex justify-center pb-safe sticky bottom-0 z-50 px-4 [--dock-item-size:2.75rem] sm:[--dock-item-size:3.5rem]", className)} {...props}>
...
export function DockItem({
  ...
  return <div ref={ref} onMouseEnter={() => setActiveIndex(index)} onMouseLeave={() => setActiveIndex(null)} className={cn("group relative flex aspect-square cursor-pointer flex-col items-center justify-center transition-all", {
...
export function DockIcon({
  ...
  return <div className={cn("flex h-full w-full items-center justify-center rounded-xl bg-background/80 shadow-sm dark:bg-gray-800/80 transition-all", className)} {...props}>
...
export function DockLabel({
  ...
  const isActive = activeIndex === index;
  return <span ref={ref} className={cn("pointer-events-none absolute -top-8 rounded-md bg-popover px-2 py-1 text-sm font-medium shadow-sm text-popover-foreground opacity-0 transition-opacity", isActive && "opacity-100", className)} {...props}>
      {children}
    </span>;
}